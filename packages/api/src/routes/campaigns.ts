...
const createSchema = z.object({
  ...
  sending_domain: z.string().max(50).optional(),
  warmup_enabled: z.boolean().optional(),
  warmup_config: z.record(z.unknown()).optional().nullable(),
  ...
  app.post('/', async (request, reply) => {
    ...
    const campaign = await db
      ...
      .values({
        ...
        sending_domain: body.sending_domain ?? 'news',
        warmup_enabled: body.warmup_enabled ?? false,
        warmup_config: body.warmup_config ?? null,
        tags: body.tags ?? [],
